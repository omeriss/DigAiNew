...
vector<double> NeuralNetwork::Calc(double* input)
{
...
void NeuralNetwork::SaveToFile(string fileName)
{
	...
	int len = bias[0].size();
	for (int j = 0; j < len; j++) {
		file.write((const char*)&(bias[0][j]), sizeof(double));
	}
	for (int i = 0; i < NumOfNodesInHidden; i++) {
		for (int j = 0; j < NumOfNodesInInput; j++) {
	...
	// mod from hidden to hidden
	for (int k = 0; k < NumOfHiddenLayers - 1; k++) {
		int len = bias[k+1].size();
		for (int j = 0; j < len; j++) {
			file.write((const char*)&(bias[k+1][j]), sizeof(double));
		}
		for (int i = 0; i < NumOfNodesInHidden; i++) {
			for (int j = 0; j < NumOfNodesInHidden; j++) {
	...
	len = bias[NumOfHiddenLayers].size();
	for (int j = 0; j < len; j++) {
		file.write((const char*)&(bias[NumOfHiddenLayers][j]), sizeof(double));
	}
	// mod last hidden to output
	for (int i = 0; i < NumOfNodesInOutput; i++) {
		for (int j = 0; j < NumOfNodesInHidden; j++) {
...
void NeuralNetwork::BackProp(vector<double*> input, vector<int> label, double jumpSize)
{
	...
	for (int t = 0; t < SetLen; t++) {
		...
		auto output = Calc(input[t]);
		int maxn = 0;
		for (int i = 1; i < output.size(); i++) {
			if (output[i] > output[maxn])
				maxn = i;
		}
		std::cout << label[t] << " " << maxn << "\n";
