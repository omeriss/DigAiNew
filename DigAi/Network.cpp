...
Network::Network(std::vector<Layer*> layers, size_t inputSize, size_t outputSize, std::string path)
{
...
std::vector<NetworkType> Network::FeedForword(NetworkType* input)
{
...
void Network::Backpropagation(NetworkType* input, int label, std::function<NetworkType* (std::vector<NetworkType>, int)>& costDerivative)
{
...
void Network::Learn(std::vector<NetworkType*>& input, std::vector<int>& label, std::function<NetworkType* (std::vector<NetworkType>, int)> costDerivative, int epochs, double learnRate, int setSize, std::string savePath)
{
	for (auto layer : layers)
		layer->SetUpLearning();
